...
class AddMoneyViewCaptain extends StatefulWidget {
  const AddMoneyViewCaptain({super.key});

  @override
  State<AddMoneyViewCaptain> createState() => _AddMoneyViewCaptainState();
}

class _AddMoneyViewCaptainState extends State<AddMoneyViewCaptain> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: MyAppBar(title: "Add Money Captain"),
      body: Column(
        children: [
          Expanded(
            child: ListView(
...
class WalletService {
  ...
  Future<String> _generateTransactionId() async {
    ...
    return 'VWID$year${counter.toString().padLeft(4, '0')}';
  }
  ...
  Future<void> addMoneyToWallet({
    ...
    final transactionRef = _firestore.collection('captain_wallet_transactions').doc();
