...
class CancellationStatusScreen extends StatefulWidget {
  ...
  @override
  State<CancellationStatusScreen> createState() => _CancellationStatusScreenState();
}
...
class _CancellationStatusScreenState extends State<CancellationStatusScreen> {
  ...
  Future<void> _loadUserTypeAndPolicies() async {
    try {
      ...
      print('Error loading cancellation policies: $e');
      setState(() {
        _isLoading = false;
        _hasError = true;
      });
  ...
  Future<void> _fetchPoliciesFromFirestore() async {
    try {
      final querySnapshot = await _firestore
          .collection('cancellationPolicies') // Different collection name
      ...
      print('Error fetching cancellation policies: $e');
      setState(() {
        _isLoading = false;
        _hasError = true;
      });
  ...
  Widget _buildPolicyItem(Map<String, dynamic> policy) {
    return Container(
      ...
      margin: EdgeInsets.only(bottom: 25), // Added margin between cards
      child: Column(
        ...
        children: [
          Row(
            children: [
              Icon(
                Icons.cancel_outlined, // Different icon for cancellation
                color: Colors.red, // Different color for cancellation
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: MyAppBar(title: 'Cancellation Policy'),
      body: _isLoading
          ? _buildShimmerLoader()
          : _hasError
          ? Center(
        child: Column(
          ...
          ? Center(
        child: Text(
          'No cancellation policy available for ${_userType == 'driver' ? 'drivers' : 'vehicle owners'}',
          style: TextStyle(
            fontSize: 18,
            fontWeight: FontWeight.bold,
            color: Colors.black87,
        ...
        child: ListView(
          ...
          children: [
            ..._policyItems.map((policy) => _buildPolicyItem(policy)),
          ],
        ),
      ),
    );
