...
class SendToBankViewCaptain extends StatefulWidget {
  const SendToBankViewCaptain({super.key});

  @override
  State<SendToBankViewCaptain> createState() => _SendToBankViewCaptainState();
}

class _SendToBankViewCaptainState extends State<SendToBankViewCaptain> {
  ...
  Future<void> _fetchWalletData() async {
    try {
      ...
      // Get wallet balance
      final walletDoc =
      await _firestore.collection('captain_wallets').doc(userId).get();
  ...
  Future<void> _processWithdrawal() async {
    ...
    try {
      ...
      if (mounted) {
        ...
        Navigator.pushNamed(context, Routes.successTransferCaptain);
      }
    } catch (e) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: MyAppBar(title: "Send To Bank Captain"),
      body: Column(
        children: [
          Expanded(
            child: ListView(
...
class WalletService {
  ...
  Future<void> createWithdrawalRequest({
    ...
    final withdrawalRef =
    _firestore.collection('captain_wallet_transactions').doc();
    batch.set(withdrawalRef, withdrawalData);
    final walletRef = _firestore.collection('captain_wallets').doc(userId);
